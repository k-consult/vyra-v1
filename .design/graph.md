...
| `REQUIRES_CAPA` | Finding → CAPA | Finding triggers corrective action |
...
| `LOCATED_AT` | Asset → Facility | Physical location of asset |
...
| `RESULTS_IN` | Finding → Risk | Finding creates risk exposure |
| `ANALYSES` | RCA → Finding | RCA traces root cause of finding |

### Assurance Graph
| Relationship | From → To | Meaning |
|---|---|---|
| `PRODUCES` | Task → Evidence | Task execution generates evidence |
| `CAPTURED_FROM` | Evidence → Incident | Evidence originates from this incident |
...
```cypher
MATCH (inc:Incident)-[:INVOLVES]->(ast:Asset)-[:LOCATED_AT]->(fac:Facility)
...
```cypher
MATCH (fnd:Finding)-[:REQUIRES_CAPA]->(capa:CAPA)
...
| finding_rca.csv | 15 | RCA → ANALYSES → Finding |
| rca_capa.csv | 20 | Finding → REQUIRES_CAPA → CAPA |
