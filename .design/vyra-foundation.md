...
| **Applicability is an explicit, inspectable edge**, not implicit | `Asset -[:COVERED_BY]-> Control -[:IMPLEMENTS]-> Obligation` — a gap (an asset with no covering control) is a graph pattern, not a spreadsheet VLOOKUP |
...
| **Evidence is generated, not collected** | `EvidencePackage / Attestation / AssuranceStatement / Audit` as graph nodes derived from the actual chain of Obligation→Control→Signal→Decision — audit-ready export is a query over real provenance, not a document someone assembled the week before the audit |
