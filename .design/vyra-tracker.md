...
| | | Contracts | 🟢 live | `Contract` (Operational graph, 12, from `20_Vendor_Master`) + a second `Vendor` batch (`:Enterprise`, 12) — real `WITH_VENDOR`/`COORDINATED_BY`/`COVERS` edges to `Vendor`/`Role`/`Facility`, all from existing Phase 2 data. `GET /enterprise/{vendors,contracts}` + UI at `/enterprise/contracts` (linked from Landscape nav). No `Contract -> Control` link (no clean FK for it) |
...
| L5 | **Oversight** | Deviation Alerts | 🟢 live | Phase 5 — `signal-intelligence` agent watches `Signal`s directly (`fetchUnassessedSignals`, filtered on no `Decision` yet), reasons via the same local Ollama LLM as `control-intelligence`, writes `Decision` nodes (`type: 'deviation-assessment'`). Verified live against both a covered-asset and a coverage-gap-asset signal; idempotent on re-run. Phase 9 put this (and the other 3 agents) on a continuous poll (`agents/scheduler.ts`, default 60s) instead of manual CLI invocation only. |
