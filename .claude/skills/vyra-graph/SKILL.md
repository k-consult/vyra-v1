...
**Global Compliance Catalog** (`vyra-implementation-plan.md` Phase 0.5 + Phase 1, 2026-07-22): `Authority`/`Jurisdiction`/`Regulation`/`Standard`/`Clause`/`Obligation` are now seeded by a second entrypoint, `cli/orchestration/catalog-sync.ts` (reads `cli/domains/catalog/ingest-hints.json`, source data generated by `cli/scripts/convert-catalog-seed.ts` from `.design/__ref/synthetic-data/data.csv`). Every node it writes carries an extra `:Catalog` label (e.g. `(:Regulation:Catalog)`) — this is the origin-labeling convention: it scopes the sync job's writes and lets callers distinguish centrally-synced reference data from enterprise-authored data of the same label. `Framework`/`Document`/`Section` from the original chain sketch were **not** added — the actual source data has no document/section granularity (`Clause` points straight at `Regulation` or `Standard`).
...
| Authority | `Authority` | id, name, abbreviation, authorityType, jurisdictionId — fed via `:Catalog`, not in the original blueprint's Knowledge list but clearly Knowledge-domain reference data |
...
| Obligation | `Obligation` | id, name, obligationType, mandatory, version — sourced from `06_Obligations` (`ObligationID → id`), named to match the source data and ISO 37301 terminology (renamed from `Requirement` 2026-09-09) |
...
RETURN path
```
Matches `api/modules/knowledge/repo.ts`'s `TRACE_FORWARD`. **All four legs are now live** — `BELONGS_TO`/`DEFINED_BY` were dormant until the catalog sync seeded `Clause`/`Obligation`; `IMPLEMENTS`/`AGAINST` require an enterprise `Control` whose `obligationId` matches a catalog `Obligation` id to complete the last hop (none do yet, so full 4-hop paths are still empty — 3-hop `Regulation`→`Clause`→`Obligation` resolves today via `api/modules/catalog/repo.ts`'s `traceObligations`).
...
| Schedule | `Schedule` | id, cadenceUnit, cadenceInterval, anchorDate, obligationId — in `v2.ts`, not fed (see note) |
...
```
`INVOLVES`, `SUPPLIED_BY`, `OCCURRED_AT` are all live. `Signal -[:EMITTED_BY]-> Asset` is declared in `v2.ts` but dormant (no Signal feed yet, roadmap Phase 3). There is no Signal→Evidence or Evidence→Obligation edge anywhere in the codebase — `GENERATES`/`CREATES`/`VALIDATES` below were aspirational and have been removed; don't treat them as implemented.
...
RETURN path
```
Matches `api/modules/knowledge/repo.ts`'s `TRACE_REVERSE`. `AGAINST`/`IMPLEMENTS`/`DEFINED_BY`/`BELONGS_TO` are all live now (see Knowledge Graph note above) — full 4-hop paths still need an enterprise `Control.obligationId` pointing at a catalog `Obligation` id, which none do yet. Separately, `Risk -[:RAISED_BY]-> Finding` is the live edge for Finding→Risk (reversed from how it reads) — there is no `RESULTS_IN` or `MITIGATES` relationship anywhere in the codebase; those have been removed from this doc.
...
RETURN att, ep, stmt
```
`BACKED_BY` and `DERIVED_FROM` are declared in `v2.ts` but dormant — `Attestation`/`AssuranceStatement`/`EvidencePackage` have no CSV feed yet. There is no Evidence→EvidencePackage or AssuranceStatement→Obligation edge anywhere in the codebase — `SUPPORTS`/`BACKS`/`PRODUCES`/`COVERS` were aspirational and have been removed; `Certification`/`AuditFinding`/`TrustArtifact` below are blueprint-only with no relationship type assigned.

---

## Canonical Relationship Types

Ground truth is `cli/semantic-contract/contracts/v2.ts` (write path) cross-checked against `cli/domains/grc/ingest-hints.json` (dedicated edge CSVs) and every `api/modules/*/repo.ts` (read path) — see `.design/vyra-graph-spine.md` for the full reconciliation. Three tiers:
...
| `IMPLEMENTS` | Control → Obligation | Knowledge | Control satisfies an obligation |
...
| `DEFINED_BY` | Obligation → Clause | Knowledge | Obligation defined by this clause — `:Catalog` |

**Declared in `v2.ts`, not yet fed** — correct name/direction, but dormant until the target type gets a CSV feed (`vyra-implementation-plan.md` Phase 1–3):

| Relationship | From → To | Fed by |
|---|---|---|
| `PART_OF` | Task → Workflow, Workflow → Program | Phase 1/2 |
| `EMITTED_BY` | Signal → Asset | Phase 3 |
| `APPLIES_TO` | Schedule → Obligation | deferred — source data (`13_Schedule_Rules`) is Task-keyed, not Obligation-keyed; see Execution Graph note above |
| `BACKED_BY` | Attestation → EvidencePackage | — |
| `DERIVED_FROM` | AssuranceStatement → Attestation | — |
| `WAIVES` | Exception → Obligation | — |

**Named in blueprint only** — `Week`, `Approval` (Execution), `System`, `Application`, `Identity`, `Event` (Operational), `Recommendation`, `Investigation`, `Impact`, `Prediction` (Intelligence), `Certification`, `AuditFinding`, `TrustArtifact` (Assurance) appear in `.design/__ref/blueprint.md` Ch.18 (retired, historical) but have no node type in `v2.ts` and no relationship type assigned. Don't invent a relationship name for these ad hoc — that's a schema decision to raise explicitly (`vyra-implementation-plan.md` sequences most of them into Phase 3+).
