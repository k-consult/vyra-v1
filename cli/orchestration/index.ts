...
import hints from '../domains/grc/ingest-hints.json';
...
async function main() {
    log.info(`Vyra Ingestion Pipeline v${v2.version}`);
...
main().catch(err => {
    log.error(`[orchestration] pipeline failed: ${err.message}`);
    process.exit(1);
});
